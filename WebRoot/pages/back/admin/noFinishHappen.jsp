	...
	<table class="tablelist">
		...
		<tbody>
			...
			<tr>
				...
				<td>${list.ordcount }</td>
				<td>${list.orddate }</td>
				<td><strong>${list.ordpaystate }</strong><br/>&nbsp;&nbsp;&nbsp;<strong>${list.ordhapstate }</strong></td>
				<c:if test="${list.ordpaystate=='已付款'}">
				<td><a href="upDateHappen.action?ordnumber=${list.ordnumber}"><strong>去发货</strong></a></td>
				</c:if>
				<c:if test="${list.ordpaystate=='未付款'}">
				<td><a href="#?ordnumber=${list.ordnumber}"><strong>联系买家</strong></a></td>
				</c:if>
				</tr>
			</c:forEach>
			 </c:if>
	...
	<div class="ui_tb_h30">
			<table border="1px" width="100%">
				<tr>
					...
					<a href="findAllFinishPaid.action?pageSize=${requestScope.pSize-1}"><input type="button" value="上一页" class="ui_input_btn01" /></a>
					</c:if>
					<c:if test="${pSize<page }">
					<a href="findAllFinishPaid.action?pageSize=${requestScope.pSize+1}"><input type="button" value="下一页" class="ui_input_btn01"/></a>
					<a href="findAllFinishPaid.action?pageSize=${requestScope.page}"><input type="button" value="尾页" class="ui_input_btn01" /></a>	
					</c:if>					
					</div>
					</td>
				</tr>
