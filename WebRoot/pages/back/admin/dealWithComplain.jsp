  ...
  <body>
   <div class="place">
    ...
    <table class="tablelist">
        ...
        <td>${list.comdate}</td>
        <td>${list.comstate}</td>
        <td><a style="color: red" href="pages/back/admin/dealWith.jsp?complainid=${list.complainid}"><strong>去处理</strong></a></td>
        </tr>
        </c:forEach>
        </c:if>
    	<c:if test="${count<=0}">
    ...
    <div class="ui_tb_h30">
			<table border="1px" width="100%">
				<tr>
					...
					<div class="ui_frt">
					<c:if test="${pSize>1 }">						
					<a href="getAllComplainNoFinish.action?pageSize=1"><input type="button" value="首页" class="ui_input_btn01" /></a>
					<a href="getAllComplainNoFinish.action?pageSize=${requestScope.pSize-1}"><input type="button" value="上一页" class="ui_input_btn01" /></a>
					</c:if>
					<c:if test="${pSize<page }">
					<a href="getAllComplainNoFinish.action?pageSize=${requestScope.pSize+1}"><input type="button" value="下一页" class="ui_input_btn01"/></a>
					<a href="getAllComplainNoFinish.action?pageSize=${requestScope.page}"><input type="button" value="尾页" class="ui_input_btn01" /></a>	
					</c:if>					
					</div>
					</td>
				</tr>
