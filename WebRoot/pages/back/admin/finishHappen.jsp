	...
	<table class="tablelist">
		...
		<tbody>
			...
			<tr>
				...
				<td>${list.ordcount }</td>
				<td>${list.orddate }</td>
				<td><strong>${list.ordhapstate }</strong></td>
			</tr>
			</c:forEach>
			</c:if>
    		<c:if test="${count<=0}">
	...
	<div class="ui_tb_h30">
			<table border="1px" width="100%">
				<tr>
					...
					<div class="ui_frt">
					<c:if test="${pSize>1 }">						
					<a href="findAllFinishHappen.action?pageSize=1"><input type="button" value="首页" class="ui_input_btn01" /></a>
					<a href="findAllFinishHappen.action?pageSize=${requestScope.pSize-1}"><input type="button" value="上一页" class="ui_input_btn01" /></a>
					</c:if>
					<c:if test="${pSize<page }">
					<a href="findAllFinishHappen.action?pageSize=${requestScope.pSize+1}"><input type="button" value="下一页" class="ui_input_btn01"/></a>
					<a href="findAllFinishHappen.action?pageSize=${requestScope.page}"><input type="button" value="尾页" class="ui_input_btn01" /></a>	
					</c:if>					
					</div>
					</td>
				</tr>
